...
# Plot Original Graph
{
  ...
  V(graph)$color <- color(1.0*(1 - outer)) # White if outer, black if not
...
#Perturbate Graph and plot it
{
  fn <- 0.22
  smpl <- sample(x= 1:dim(A)[1], size = floor(dim(A)[1] * fn), replace = FALSE)
  ...
  colorarray <- rep(0.5, dim(A)[1])
  colorarray[which(outer == 0)] <- 1.0
  colorarray[smpl] <- 0.0

  outer[smpl] <- 1 # If we remove an outer node, it's expression can (and will) drop.

  graph <- igraph::graph_from_adjacency_matrix(A)

  V(graph)$color <- color(colorarray) # White if outer, black if not
  V(graph)$border.color <- "white"
...
}, movie.name = "~/Documents/Work/SysBio2023/Presentations/network-flow-at-post-perturbation.gif", interval = 0.1)
